 ...
 * @author sakekasi,cj
 */
public class AutonomousFileManager {

    private static AutonomousFileManager _INSTANCE;

    protected AutonomousFileWriter writer = null;
    protected AutonomousFileReader reader = null;
    boolean write;
    boolean read;
    String path;

    private AutonomousFileManager() throws IOException{
        this.reset();
    }

    public static AutonomousFileManager getInstance() throws IOException{
        if(_INSTANCE == null){
            _INSTANCE = new AutonomousFileManager();
        }
        return _INSTANCE;
    }
...
//        throw new IllegalStateException("WRITING NOT ENABLED");
    ...
    public void reset(){
        ...
        try{
            if (this.write)
                this.writer = new AutonomousFileWriter(this.path);
        }catch(Exception e){
            this.writer = null;
            this.write = false;
        ...
        try{
            if (this.read)
                this.reader = new AutonomousFileReader(this.path);
        }catch(Exception e){
            this.reader = null;
            this.read = false;
